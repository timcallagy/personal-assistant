...
import type { BlogCategory, TagWithCount, BlogConfig } from '@/lib/api';
          ...
          {categories.map((cat) => (
            <li key={cat.id}>
              <Link
                ...
                className="flex justify-between items-center text-secondary hover:text-accent transition-colors"
              >
                <span>{cat.name}</span>
                <span className="text-muted text-sm">({cat.postCount})</span>
              </Link>
            </li>
          ))}
        </ul>
