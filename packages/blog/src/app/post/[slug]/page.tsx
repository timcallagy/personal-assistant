    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <article className="flex-1 max-w-none">
          ...
          {/* Meta */}
          <div className="entry-meta mb-8 pb-8 border-b border-border">
          ...
          {post.tags.length > 0 && (
            <div className="mt-8 pt-8 border-t border-border">
              <span className="text-muted text-sm mr-2">Tags:</span>
              <div className="inline-flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/tag/${encodeURIComponent(tag)}`}
                    className="px-3 py-1 bg-surface text-secondary text-sm hover:bg-accent hover:text-white transition-colors"
                  >
                    {tag}
                  </Link>
                ))}
          ...
          <nav className="mt-12 pt-8 border-t border-border">
            <div className="flex flex-col sm:flex-row justify-between gap-4">
              {post.previousPost ? (
                ...
                >
                  <span className="text-muted text-sm">Previous</span>
                  <p className="text-secondary group-hover:text-accent transition-colors line-clamp-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {post.relatedPosts.map((related) => (
                  ...
                  >
                    ...
                    <p className="text-secondary group-hover:text-accent transition-colors line-clamp-2">
                      {related.title}
                    </p>
                  </Link>
                ))}
